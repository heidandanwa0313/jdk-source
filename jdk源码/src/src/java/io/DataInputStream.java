 ...
 * @since   JDK1.0
 */
 //DataInputStream 的作用就是“允许应用程序以与机器无关方式从底层输入流中读取
 //基本 Java 数据类型。应用程序可以使用数据输出流写入，稍后由数据输入流读取的数据。”
public
class DataInputStream extends FilterInputStream implements DataInput {
     ...
     * @see        java.io.FilterInputStream#in
     */
     //从数据输入流中读取数据并填满字节数组b；没有填满数据b时，则一直读取，直至填满位置
	...
	int ch = in.read();
	if (ch < 0)
	    throw new EOFException();    //流已经到达最后，则报出异常
	return ch;
    }
